...
ggplot(data, aes(PC1, PC2, color = strain, fill = strain)) + geom_point(aes(shape = sex)) + scale_shape_manual(values = c(10,3))+ stat_ellipse(type = "norm", geom = "polygon", alpha = 0.35, level = 0.5,aes(fill = strain)) + labs(x = "PC1 (20.2% var. explained)", y = "PC2 (15.2% var. explained)")+ ggtitle("PCA of Transcriptome of Control/Dsx Mutant")
...
## fix bug: too few points to calc an ellipse 
